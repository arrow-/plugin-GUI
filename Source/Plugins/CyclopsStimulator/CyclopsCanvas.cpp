...
int CyclopsCanvas::migrateEditor(CyclopsCanvas* dest, CyclopsCanvas* src, CyclopsCanvas::Listener* listener)
{
...
int CyclopsCanvas::migrateEditor(CyclopsCanvas* dest, CyclopsCanvas* src, int nodeId)
{
    ...
    return CyclopsCanvas::migrateEditor(dest, src, listener);
}
...
{  
    ...
    else if (button == closeButton)
    {
        ...
        case 1: // migrate
            ...
            DialogWindow* the_window = dw.launchAsync();
        break;
        }
    }
}
...
{
    std::cout << "vac " << newVisibleArea.getWidth() << " " << newVisibleArea.getHeight() <<std::endl;
}
